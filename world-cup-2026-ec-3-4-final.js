const getWorldCupMatches = async (
  ...
  const getCardTitle = (matchStage, matchDate) => {
    /*const title =
      round === roundStage && matchGroup
        ? `Grupo ${matchGroup}`
        : getRound(round);*/

    return `${matchStage} • ${formatDate(matchDate)}`;
  };
  ...
  const renderMatch = (match) => {
    ...
    article.innerHTML = `
      <header class="${classes.fixtureCardTopHeader}">
        <span class="${classes.fixtureCardGroup}">${getCardTitle(match.fase ?? "", match.fecha)}</span>
 ...
 const sortMatches = (matchList) => {
  return [...matchList].sort((a, b) => {
    ...
    // Mismo grupo → orden cronológico por fecha + horaLima
    const dateTimeA = `${a.fecha ?? "0000-00-00"} ${a.horaLima ?? "00:00"}`;
    const dateTimeB = `${b.fecha ?? "0000-00-00"} ${b.horaLima ?? "00:00"}`;
